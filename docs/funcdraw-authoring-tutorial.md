...
## 2. Install and configure `@funcdraw/fd-play`
`fd-play` is the CLI that packages your FuncDraw expressions and serves them to a browser-based player. Install it as a dev dependency and add a script named `play` to `package.json`:
...
`fd-play` bundles every `.fs` (or `.js`) expression under the current folder, starts a dev server on port `4123`, and opens your default browser to the FuncDraw Player. The player evaluates `main.fs` with the framing from `view.fs`, so you will see the teal circle centered at the origin. Leave the server running: each time you edit `main.fs` or `view.fs`, the browser automatically reloads, giving you instant feedback as you iterate on shapes, colors, or animation parameters.
