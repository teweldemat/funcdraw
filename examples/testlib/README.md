...
- `art/cartoon/stickman/` – the stick man model from the JavaScript example. Call `package("@funcdraw/testlib").cartoon.stickman.static` for the classic pose, `package("@funcdraw/testlib").cartoon.stickman.steperManProfile` for the side-view walk cycle with explicit foot targets, or `package("@funcdraw/testlib").cartoon.stickman.steperManZoom` for a zoom-in/out stride that alternates limb length.
