...
3. **Simulate step-by-step** – for each stride up to the current progress, call `singleStepProfile` with the current moving foot target (plus the current anchor/leg offsets) and accumulate the resulting anchor so the torso and feet translate smoothly.
...
type MultiStepProfileOptions = {
  ...
  handSwing?: SteperManProfileOptions["handSwing"]; // optional arm swing settings forwarded to singleStepProfile
};
```
