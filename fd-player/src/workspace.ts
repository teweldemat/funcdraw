...
const sanitizeCustomTabs = (value: unknown): CustomTabState[] | undefined => {
  ...
  for (const entry of value) {
    ...
    if (typeof id === 'string' && typeof name === 'string' && typeof expression === 'string') {
      ...
      fallbackCounter += 1;
      let language: ExpressionLanguage | undefined;
      if ('language' in entry) {
        const candidate = (entry as { language?: unknown }).language;
        if (typeof candidate === 'string') {
          language = candidate as ExpressionLanguage;
        }
      }
      result.push({ id, name, expression, folderId, createdAt, language });
    }
  }
  return result;
};
...
export const loadPersistedSnapshot = (storageKey: string = STORAGE_KEY): PersistedSnapshot | null => {
  ...
  try {
    ...
    if (typeof data.projectName === 'string') {
      snapshot.projectName = data.projectName;
    }
